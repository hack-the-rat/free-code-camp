...
```<p>Read more on the <a href="/Users/user/Desktop/fCC/script-code/absolute-vs-relative-paths/pages/about.html">About Page</a></p>```
...
So imagine you are on the `contact.htm`l page, and because the `about.html` page is in the same place, you simply get the filename. This is an example of using a relative file path.
...
Take a look at the link in the [preview window](https://codesandbox.io/p/sandbox/qm722j). It should be the default color of blue which represents the default state. If you click on it, then it will turn purple.
...
Click on the link in the [preview window](https://codesandbox.io/p/sandbox/fpfcty) and you should see that the visited link changes to the color brown.
...
Hover your mouse over the link in the [preview window](https://codesandbox.io/p/sandbox/mgjrpt) and you will see the color change to red.

Then we have `:focus`. This state applies when we focus on a link.

Click on any portion of the whitespace in the [preview window](https://codesandbox.io/p/sandbox/tgzdkn) and then press `tab` on your keyboard. You should see the link change to green.
...
Click on the link in the [preview window](https://codesandbox.io/p/sandbox/pdgtxm) and you should see the link turn to black. This happens pretty quickly so you might need to click on it a few times to see the color change.
